 ...
 * @property {?Function} duplicateChecker
...
Parser.prototype = {
        ...
        return this._env
            ...
            .then(function (nodesCount) {
                ...
                return this
                    ._parseRow({
                        collection: collection,
                        duplicateChecker: rule.duplicateChecker,
    ...
    _parseRow: function (options) {
        ...
        var checker = typeof options.duplicateChecker === 'function' ?
            function (_id) {
                return options.duplicateChecker(_id, options.results);
            } : false;
        ...
        return this
            ...
            .then(function (result) {
                if (!options.duplicateChecker || result !== false) {
    ...
    _parseCollectionRule: function (rule) {
        ...
        var self = this,
            ...
            duplicateChecker = rule.duplicateChecker,
            needDuplicateCheck = typeof rule.duplicateChecker === 'function',
            results = {};

        if (needDuplicateCheck) {
            // put the _id rule first in the collection
            var index = collection.findIndex(function (item) {
                return !!item.id;
            });
            if (index !== -1) {
                var found = collection.splice(index, 1).pop();
        ...
        return collection.reduce(function (promise, rule) {
            return promise
                ...
                .then(function (parsed) {
                    if (needDuplicateCheck && parsed.name === '_id') {
                        return duplicateChecker(parsed.value);
                    }
                    return vow.resolve(false);
                })
                .then(function (duplicateDetected) {
                    if (duplicateDetected) {
                        var error = new Error('Duplicate row detected. Skip parsing.');
                        error.name = 'DuplicationException';
                        throw error;
                    }
                });
        }, vow.resolve())
            .catch(function (error) {
                if (error.name !== 'DuplicationException') {
                    throw error;
                }
                debug('Skip duplicate row');
                results = false;
            })
            .then(function () {
